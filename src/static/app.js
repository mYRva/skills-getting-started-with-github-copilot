      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (participants.length) {
          participantsHtml = `<div class="participants" aria-label="participants"><strong>Participants:</strong><ul class="participants-list">` +
            participants.map((p) => `<li title="${escapeHtml(p)}"><span class="avatar">${getInitials(p)}</span><span class="participant-email">${escapeHtml(p)}</span><button class="remove-participant" data-email="${escapeHtml(p)}" data-activity="${escapeHtml(name)}" aria-label="Remove ${escapeHtml(p)}">✕</button></li>`).join("") +
            `</ul></div>`;
        }
